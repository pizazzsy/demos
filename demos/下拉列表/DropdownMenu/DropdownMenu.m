...
- (void)createMainBtnWithFrame:(CGRect)frame andTitle:(NSString*)title{
    ...
    _arrowMark.center = CGPointMake(_arrowMark.center.x, _mainBtn.size.height/2);
...
- (void)setMenuTitles:(NSArray *)titlesArr rowHeight:(CGFloat)rowHeight{
    ...
    _listView.layer.borderColor   = [UIColor colorWithHexString:@"e9e9e9"].CGColor;
    _listView.layer.borderWidth   = 0.5f;
    
    
    // 下拉列表TableView
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0,_listView.size.width, _listView.size.height)];
...
- (void)showDropDown{   // 显示下拉列表
    ...
    [UIView animateWithDuration:AnimateTime animations:^{
        ...
        _listView.frame  = CGRectMake(_listView.frame.origin.x, _listView.frame.origin.y, _listView.frame.size.width, _rowHeight *_titleArr.count);
